...
const NFT = () => {
  const [currentAccount, setCurrentAccount] = useState(localStorage.getItem("userAddress") || "");
  const [connectedAccount, setConnectedAccount] = useState(false);
  const [open, setOpen] = useState(false);

  const [currentWallet, setCurrentWallet] = useState(localStorage.getItem("userWallet") || "");
  ...
  return (
    <>
      <Navbar activeAccount={connectedAccount} currentAccount={currentAccount} connectWallet={openModal} disconnectActiveAccount={disconnectAccount} />
      <WalletModal closeModal={closeModal} showModal={open} connectMetaMask={isMobile ? connectWalletConnect : connectMetaMask} connectWalletConnect={connectWalletConnect} />
      <NFTHero />
      <NFTBody />
      <NFTBottom activeAccount={connectedAccount} currentAccount={currentAccount} />
    </>
  );
};
